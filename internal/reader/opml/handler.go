	...
	for _, subscription := range subscriptions {
		if !h.store.FeedURLExists(ctx, userID, subscription.FeedURL) {
			...
			if subscription.CategoryName == "" {
				...
				category, err = h.store.CategoryByTitle(ctx, userID, subscription.CategoryName)
				if err != nil {
					return fmt.Errorf("opml: unable to search category by title: %w", err)
				}

				if category == nil {
					category, err = h.store.CreateCategory(ctx, userID, &model.CategoryCreationRequest{Title: subscription.CategoryName})
					if err != nil {
