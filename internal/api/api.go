...
func Serve(router *mux.Router, store *storage.Storage, pool *worker.Pool) {
	...
	sr.HandleFunc("/entries/{entryID}/user-tags", handler.setEntryUserTags).Methods(http.MethodPut)
	sr.HandleFunc("/user-tags", handler.getUserTags).Methods(http.MethodGet)
	sr.HandleFunc("/user-tags", handler.createUserTag).Methods(http.MethodPost)
	sr.HandleFunc("/user-tags/{userTagID}", handler.updateUserTag).Methods(http.MethodPut)
	sr.HandleFunc("/user-tags/{userTagID}", handler.removeUserTag).Methods(http.MethodDelete)
	sr.HandleFunc("/user-tags/{userTagID}/entries", handler.getUserTagEntries).Methods(http.MethodGet)
