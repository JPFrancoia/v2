...
func (h *handler) showToReviewEntryPage(w http.ResponseWriter, r *http.Request) {
	...
	if entry == nil {
		response.HTMLRedirect(w, r, h.routePath("/to-review"))
		return
	}
	...
	nextEntryRoute := ""
	if nextEntry != nil {
		nextEntryRoute = h.routePath("/to-review/entry/%d", nextEntry.ID)
	}

	prevEntryRoute := ""
	if prevEntry != nil {
		prevEntryRoute = h.routePath("/to-review/entry/%d", prevEntry.ID)
	}
